...
Meteor.methods({
  ...
  invite: function (partyId, userId) {
    ...
    if (! party || party.owner !== this.userId)
      throw new Meteor.Error(404, "No such meet");
    if (party.public)
      throw new Meteor.Error(400,
                             "That meet is public. No need to invite people.");
    if (userId !== party.owner && ! _.contains(party.invited, userId)) {
      ...
      if (Meteor.isServer && to) {
        ...
        Email.send({
          from: "nospam@truevis.com",
          to: to,
          replyTo: from || undefined,
          subject: "Meet: " + party.title,
          text:
"Hey, I just invited you to '" + party.title + "' on the conference meet app." +
"\n\nCome check it out: " + Meteor.absoluteUrl() + "\n"
  ...
  rsvp: function (partyId, rsvp) {
    ...
    if (! _.contains(['yes', 'no', 'maybe'], rsvp))
      throw new Meteor.Error(400, "Invalid reply");
    var party = Parties.findOne(partyId);
    if (! party)
      throw new Meteor.Error(404, "No such meet");
    if (! party.public && party.owner !== this.userId &&
      ...
      throw new Meteor.Error(403, "No such meet");
